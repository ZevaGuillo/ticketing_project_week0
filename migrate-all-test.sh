...
migrate_service() {
    ...
    # Restaurar paquetes NuGet - ir al directorio del servicio donde está la .sln
    echo "   - Restaurando paquetes NuGet..."
    service_base_dir="$startup_project_path/../.."
    echo "   - Directorio del servicio: $service_base_dir"
    cd "$service_base_dir"
    echo "   - Directorio actual después de cd: $PWD"
    
    # Buscar el archivo .sln en el directorio actual
    sln_file=$(find . -maxdepth 1 -name "*.sln" | head -n 1)
    
    if [ -n "$sln_file" ]; then
        echo "   - Usando solución: $sln_file"
        ...
        echo -e "${RED}   ❌ No se encontró archivo .sln en $service_base_dir${NC}"
    ...
    # Asegurar que el proyecto Infrastructure tenga sus dependencias restauradas
    echo "   - Restaurando dependencias del proyecto Infrastructure..."
    if dotnet restore "$infrastructure_project_path" --verbosity quiet; then
        echo "   - Dependencias de Infrastructure restauradas"
    else
        echo -e "${YELLOW}   ⚠️  Advertencia: No se pudieron restaurar algunas dependencias del proyecto Infrastructure${NC}"
    fi
